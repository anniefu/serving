...
func (ae *Exporter) Start() error {
	...
	ae.startOnce.Do(func() {
		...
		if err := ae.connect(); err == nil {
			fmt.Println("ANNIE: Start connected")
			ae.setStateConnected()
		} else {
			fmt.Println("ANNIE: Start disconnected")
			ae.setStateDisconnected(err)
		}
		log.Println("ANNIE: starting indefinite background connection")
...
func (ae *Exporter) Stop() error {
	log.Print("ANNIE: ocagent exporter Stop called")
	ae.mu.RLock()
