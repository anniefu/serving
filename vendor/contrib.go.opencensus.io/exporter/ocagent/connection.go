...
func (ae *Exporter) indefiniteBackgroundConnection() error {
	defer func() {
		log.Println("ANNIE: ending indefinite background connection")
	...
	for {
		log.Println("ANNIE: for loop start")
		...
		select {
		case <-ae.stopCh:
			log.Println("ANNIE: WAHT")
			return errStopped

		case <-ae.disconnectedCh:
			// Normal scenario that we'll wait for
			log.Println("ANNIE: WAHT 4")
		}

		if err := ae.connect(); err == nil {
			log.Println("ANNIE: connection succcesful")
		...
		select {
		case <-ae.stopCh:
			log.Println("ANNIE: WAHT 2")
			return errStopped
		case <-time.After(connReattemptPeriod + jitter):
			log.Println("ANNIE: WAHT 3")
		}
	}
